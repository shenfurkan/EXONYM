...
def test_survey_search_routes_only_to_human_review(tmp_path, monkeypatch):
    ...
    def fake_search(candidate, sectors=None, result_suffix=None):
        calls.append(
            {"candidate": candidate.candidate_id, "sectors": sectors, "result_suffix": result_suffix}
        )
    ...
    record = json.loads(record_path.read_text(encoding="utf-8"))
    assert calls == [
        {"candidate": workspace.candidate_id, "sectors": [17], "result_suffix": ".survey-test-survey"}
    ]
...
def test_survey_search_reuses_matching_result_after_interrupted_record_write(tmp_path, monkeypatch):
    ...
    (outputs / "bls_search_manifest.survey-test-survey.json").write_text(
        json.dumps({"configuration": {"engine": "bls", "signal": None, "sectors": [17]}}),
        encoding="utf-8",
    )

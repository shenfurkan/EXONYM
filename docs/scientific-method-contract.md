...
The function lists are public or externally meaningful entry points; private
helpers inherit the same units and limits from their row. Each routine raises
`ValueError`/`RuntimeError` or returns an explicitly unavailable diagnostic when
its stated domain is not met. No row authorizes a claim.

| Modules and scientific functions | Inputs and returned units | Domain, assumptions, and hard boundary | Source key(s) |
| --- | --- | --- | --- |

"""Numerically stable helper functions for the TREX Monte Carlo integrator.

These pure-numerics routines are separated so they can be imported and
tested without pulling in the full astronomy dependency chain.

References
----------
* _log_mean_exp follows the same log(mean(exp(x))) pattern as the
...
def _log_mean_exp(logw: np.ndarray, *, N_total: int) -> float:
    ...
    ``-inf`` entries (non-transiting draws) contribute zero weight to the
    sum but still count in the denominator, preserving the same mean
    semantics as ``np.mean(np.nan_to_num(np.exp(lnL + 600)))``.

    ``NaN`` entries (numerical failures) are treated identically to
    ...
    Args:
        ...
        N_total: Total number of MC draws (must equal len(logw)).
...
def _normalize_probabilities(lnZ: np.ndarray):
    ...
    if np.any(np.isnan(lnZ)) or np.any(np.isposinf(lnZ)):
        return np.zeros(len(lnZ), dtype=float), "anomaly"
    if np.all(np.isneginf(lnZ)):
        return np.zeros(len(lnZ), dtype=float), "all_neginf"

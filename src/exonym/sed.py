...
The active runner evaluates only retained, hash-bound MIST v1.2 bolometric-
correction archives supplied below a candidate workspace.  It holds the
candidate's atmospheric parameters fixed, profiles apparent bolometric
magnitude, and fits visual extinction within the native MIST table domain.

Legacy analytic helpers remain temporarily for direct compatibility callers but
are not reachable through :func:`run_sed_fit`.  They cannot produce a
candidate-local SED result.
